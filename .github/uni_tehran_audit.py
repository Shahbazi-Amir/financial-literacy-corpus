...
'## ساختار','',
f'- فایل‌های شماره‌دار Output: **{len(outs)}** — {min(outs)} تا {max(outs)}',
f'- فایل‌های شماره‌دار متن خام: **{len(refs)}** — {min(refs)} تا {max(refs)}',
f'- فایل‌های mirror در `متن ها/uni_tehran/review/md`: **{len(reviews)}**؛ برابر byte-for-byte با Output: **{mirror_equal}/{len(outs)}**',
f'- فقط در Output: `{sorted(set(outs)-set(refs))}`',f'- فقط در متن خام: `{sorted(set(refs)-set(outs))}`','',
'## مقایسه قسمت‌به‌قسمت','',
...
for n in allnums:
    ...
    o=outs[n].read_text(encoding='utf-8'); r=refs[n].read_text(encoding='utf-8'); ow,rw=len(words(o)),len(words(r))
    op=[p for p in BAD_PATTERNS if p in o]; rp=[p for p in BAD_PATTERNS if p in r]; hs=ref_positive_spans(o,r,8)
...
for n,spans in heavy.items():
    ...
    chosen=spans if n==4 else spans[:12]
    for delta,tag,oc,rc in chosen:
